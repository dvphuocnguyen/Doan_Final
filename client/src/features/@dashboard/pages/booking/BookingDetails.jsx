...
export default function BookingDetails({ data, open, onClose }) {
  const resultCountDate = useMemo(() => {
    const result = differenceInDays(new Date(data?.end_date), new Date(data?.start_date));
      ...
      <BootstrapDialog onClose={onClose} aria-labelledby="customized-dialog-title" open={open}>
        ...
        <DialogContent dividers>
          <Typography gutterBottom>
            Đặt cho:{" "}
            <strong>{data.booking_for === "ME" ? "Chính anh ấy" : "Cho người khác"}</strong>
          </Typography>
          {data.booking_for !== "ME" && (
            <>
              <Typography gutterBottom>
          ...
          <Typography gutterBottom>
            Ngày nhận phòng: <strong>{formatDateVN(new Date(data.start_date))}</strong>
          </Typography>
          <Typography gutterBottom>
            Ngày trả phòng: <strong>{formatDateVN(new Date(data.end_date))}</strong>
          </Typography>
          <Typography gutterBottom>
          ...
          <Typography gutterBottom>
            Đã sử dụng voucher: <strong>{data.voucher || "Không có sử dụng"}</strong>
          </Typography>
          <Typography gutterBottom>
            Tổng số đêm nghỉ dưỡng: <i style={{ color: "red" }}>{`${resultCountDate} đêm`}</i>
          </Typography>
