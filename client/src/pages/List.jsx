...
function List(props) {
  ...
  useEffect(() => {
    if (!location.state?.destination || !location.state?.date || !location.state?.options) return;
    ...
    const timer = setTimeout(() => {
      dispatch(
        hotelActions.findHotelsStart({
          ...
          total_people: +location.state.options.adult + +location.state.options.children,
        })
      );
    }, 500);
      ...
      >
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={3}>
            <Paper sx={{ background: "#febb02", borderRadius: "2px", padding: 1.5 }}>
          ...
          <ListHotelSearch date={date} options={options} destination={destination} />
        </Grid>
      </Container>
