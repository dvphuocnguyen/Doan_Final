...
import { calcPriceAndTax, calcPriceDiscount } from "~/utils";
import { fPrice } from "~/utils/formatNumber";

function Hotel(props) {
  const { data, rooms, images, tags } = useSelector(hotelSelect);
  ...
  const resultCountDate = useMemo(() => {
    const result = differenceInDays(state.date[0]?.endDate, state.date[0]?.startDate);
  ...
  const handleOnClickBooking = useCallback(() => {
    ...
    if (_.isEmpty(selected)) {
      ...
      if (_.isEmpty(user)) {
        toast.error("Đăng nhập trước khi đặt phòng!");
        navigate("/sign-in");
        return;
      }
    ...
    <Page title={data.hotel_name}>
      ...
      {loading ? (
        ...
        <Container maxWidth="lg" sx={{ mt: 2, mb: 5 }}>
          ...
          {!rooms.length ? (
            <Typography color="red" sx={{ mt: 2 }} fontSize={18} fontWeight={700}>
                  ...
                  >
                    ...
                    <Stack direction="row" mt={2}>
                      ...
                      <Typography fontSize={14}>
                        Nằm tại khu vực được đánh giá cao nhất ở Hội An, khách sạn này có điểm vị
                        trí xuất sắc 9,3
                      </Typography>
                    </Stack>
                  ...
                  {rooms?.length &&
                    rooms.map((room, index) => (
                      <Grid key={index} item lg={3} md={4} xs={12}>
                        ...
                        >
                          <Typography color="#4c76b2" fontWeight={700} fontSize={18}>
                            {room.room_name}
                          </Typography>
                          ...
                          <Stack direction="row" spacing={1} mt={2} mb={2}>
                            <Typography fontWeight={600}>Phù hợp cho: </Typography>
                            <Typography fontWeight={700}>{`${room.max_people} người`}</Typography>
                          </Stack>

                          <hr style={{ borderTop: "1px solid #e7e7e7" }} />

                          <Stack direction="row" spacing={1} mt={2}>
                            <Typography fontWeight={600}>Giá hôm nay:</Typography>
                            <Typography fontWeight={700}>
                              {room.discount === 1
                                ...
                                : fPrice(
                                    calcPriceAndTax({
                                      price: room.price * resultCountDate,
                                      tax: data.tax,
                                    })
                                  )}
                            </Typography>
                          ...
                          <Typography fontStyle="italic" fontSize={12} fontWeight={700}>
                            Bao gồm {data.tax}% thuế
                          </Typography>
                          ...
                          >
                            <Typography
                              ...
                              fontWeight={500}
                            >
                              {`Chỉ còn lại ${room.room_quantity - room.room_booking} phòng`}
                            </Typography>
                          ...
                          <FormControl fullWidth margin="normal" size="small">
                            <InputLabel id="demo-simple-select-label">Số lượng phòng</InputLabel>
                            <Select
                              label="Số lượng phòng"
                              value={selected[room.room_id] || ""}
                              onChange={(e) => handleChangeSelect(e, room)}
                            >
                              {[...Array(room.room_quantity - room.room_booking)].map(
                                (i, index) => (
                                  <MenuItem value={index + 1} key={index}>
                                    <Stack direction="row" spacing={1}>
                                      <Typography>{`${index + 1} phòng.`}</Typography>
                                      <Typography>
                                        (
                                        {room.discount === 1
                                          ? fPrice(
                                          ...
                                          : fPrice(
                                              calcPriceAndTax({
                                                price: room.price * (index + 1) * resultCountDate,
                                                tax: data.tax,
                                              })
                                            )}
                                        )
                          ...
                          <Stack direction="row" justifyContent="space-between" spacing={1}>
                            <Button
                              disabled={selectedRooms[room.room_id]?.room_id === room.room_id}
                              variant="contained"
                              fullWidth
                              sx={{ borderRadius: "2px" }}
