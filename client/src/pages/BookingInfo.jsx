...
function BookingInfo(props) {
  const { paginations, filters, data, isLoading, details } = useSelector(billState);
  ...
  return (
    <Page title="Thông tin đặt phòng">
      ...
      <Container sx={{ p: 6 }} maxWidth="lg">
        <Typography variant="h5">Danh sách phòng đã đặt</Typography>
          ...
          {data && data.length ? (
            data.map((row, index) => (
              <TableRow key={index} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                ...
                <TableCell align="right">
                  <Typography fontSize={14} color={statuses[row.status].textColor}>
