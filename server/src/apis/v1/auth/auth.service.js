...
class AuthService {
  signIn({ username, password }) {
    return new Promise(async (resovle, reject) => {
      try {
        // Find username
        let sql = SqlString.format("SELECT * FROM ?? WHERE username = ?", ["users", username]);
        ...
        const { password: passwordHash, user_id, role, ...others } = { ...result[0] };
  ...
  handleForgorPassword({ username, email }) {
    return new Promise(async (resovle, reject) => {
      try {
        ...
        let sql = SqlString.format("SELECT * from `users` WHERE username=?", [username]);

        const [findCustomer] = await pool.query(sql); // [[{user_id, ....}]]

        if (!findCustomer.length) {
          return reject(new APIError(404, "Không tìm thấy tài khoản. Vui lòng nhập đúng!"));
        }

        const { user_id, last_name } = { ...findCustomer[0] };

        // check OTP exist with user_id.
        sql = SqlString.format("SELECT * FROM `tokens` WHERE user_id=?", [user_id]);

        const [findOTP] = await pool.query(sql);

        if (findOTP.length > 0) {
          return reject(new APIError(400, "Email đã tồn tại. Vui lòng kiểm tra."));
        }
        ...
        sql = SqlString.format("INSERT INTO ?? SET ?", ["tokens", { user_id, token: _hashOTP }]);
  ...
  handleChangePassword({ userId, token, password }) {
    return new Promise(async (resovle, reject) => {
      try {
        // find Token
        let sql = SqlString.format("SELECT * FROM `tokens` WHERE user_id=?", [userId]);
        ...
        sql = SqlString.format("DELETE FROM `tokens` WHERE user_id=?", [userId]);
  ...
  async handleRefreshToken(userId) {
    try {
      let sql = SqlString.format("SELECT * FROM ?? WHERE user_id=?", ["seesions", userId]);
  ...
  registerHotel(data) {
    return new Promise(async (resovle, reject) => {
      try {
        ...
        if (findEmail) {
          return reject(new APIError(400, "E-mail đã tồn tại. Hãy sử dụng email khác."));
        }
