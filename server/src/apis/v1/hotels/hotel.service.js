...
class HotelService {
  ...
  create(data = {}, user = { role: "ADMIN" }) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        sql = SqlString.format("INSERT INTO ?? SET ?", [this.table, dataInsertHotel[user.role]]);
        ...
        const listImgs = h_image_value.map((img) => [createUUID(), id, img.path, img.filename]);
         ...
         */
        const listTags = tags.map((tag) => [createUUID(), id, "tag", tag.title]);
  ...
  getById(id) {
    return new Promise(async (resolve, reject) => {
      try {
        let q = SqlString.format("SELECT * FROM ?? WHERE ??=?", [this.table, this.primaryKey, id]);

        const [result] = await pool.query(q);

        q = SqlString.format("SELECT * FROM ?? WHERE ??=?", ["hotel_images", "hotel_id", id]);

        const [images] = await pool.query(q);

        q = SqlString.format("SELECT * FROM ?? WHERE ??=?", ["hotel_tags", "hotel_id", id]);
  ...
  update(id, data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let { img_delete, tag_delete, hotel_image, h_image_value, tag_news, ...others } = data;
        ...
        if (img_delete && img_delete.length > 0) {
          ...
          q = SqlString.format("DELETE FROM `hotel_images` WHERE h_image_id IN (?)", [imgs_id]);
        ...
        if (tag_delete && tag_delete.length > 0) {
          ...
          q = SqlString.format("DELETE FROM `hotel_tags` WHERE tag_id IN (?)", [tags_id]);
        ...
        if (h_image_value && h_image_value.length > 0) {
          const listImgs = h_image_value.map((img) => [id, img.path, img.filename]);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (search && !order) {
          ...
          q = SqlString.format(sqlSwitch.qOrder[user.role].sql + orderBy + " LIMIT ? OFFSET ?", dO);
  ...
  deleteById(id) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const q = SqlString.format("DELETE FROM ?? WHERE ??=?", [this.table, this.primaryKey, id]);
  ...
  getOptions(user = { role: "ADMIN" }) {
    return new Promise(async (resolve, reject) => {
      try {
        // * dependency
        const d = user.role === roles.ADMIN ? [this.table] : [this.table, user.user_id];
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (rooms.length) {
          ...
          q = SqlString.format("SELECT `r_image_value` FROM `room_images` WHERE `room_id` IN (?)", [
            roomIds,
          ]);
