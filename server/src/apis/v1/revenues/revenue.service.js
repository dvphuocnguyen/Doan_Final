...
import bookingTripService from "../bookingTrip/bookingTrip.service.js";
import userService from "../users/user.service.js";

const statusPass = [statuses.PAID.key, statuses.ENDED_USE.key, statuses.STARTED_USE.key];
...
class RevenueService {
  ...
  static revenueByDate = async (date, user = { role: roles.ADMIN, user_id: "" }) => {
    try {
  ...
  static revenueByHotels = async (user = { role: roles.ADMIN, user_id: "" }) => {
    try {
      ...
      const data = revenueByRooms.map((result) => ({
        ...
        hotel_name:
          user.role === roles.HOTEL
            ? hotels.hotel_name
            : hotels.find((hotel) => hotel.hotel_id === result.hotel_id).hotel_name,
      }));
  ...
  static countBillCancel = async (user = { role: roles.ADMIN, user_id: "" }) => {
    try {
      let sql = SQLstring.format("SELECT COUNT(*) as total_bill_cancel FROM bills WHERE status=?", [
        statuses.CANCEL.key,
      ]);
  ...
  static selecHotel = async (ownerId = "", role = roles.ADMIN) => {
    ...
    if (role === roles.HOTEL) {
      sql = SQLstring.format("SELECT hotel_id, hotel_name FROM hotels WHERE owner_id=?", [ownerId]);
    }
  ...
  static countTrips = async () => {
    const { result: trips } = await bookingTripService.getAll();

    const costBookingTrip = trips.reduce((acc, val) => {
      return (acc += val.fee.total_price);
    }, 0);

    return costBookingTrip;
  };

  static countCustomer = async () => {
    const { result } = await userService.getAll();

    return result.filter((t) => t.role === "USER").length;
  };
}
